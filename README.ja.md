...
残りは単に未実装で、[ROADMAP.md](ROADMAP.md) で管理しています: 上限のリセットカウントダウン、
バーンレート予測、ページ付きの種図鑑、捕獲ログ、フローティングペットのホバー情報・右クリック
メニュー・吹き出し通知、ペットサイズと警告しきい値のスライダー、Keychain オプトアウト、
アプリ内アップデート確認、セーブのエクスポート・インポート。
