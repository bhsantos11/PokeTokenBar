...
バッグ／コレクション／トレーナーのウィンドウ、進化ライン、リセットまでのカウントダウンとバーンレート
予測つきの公式5時間・週間上限メーター、ページ・絞り込み・種別詳細のある種図鑑、捕獲ログ、ホバー情報・
右クリックメニュー・吹き出し通知つきのフローティングペット、デスクトップ通知、セーブの
エクスポート・インポート、アプリ内アップデート確認、設定。使用量の解析、パートナーの進行、
セーブデータは macOS と **同じコード** です。

最近の作業がここで進んだため、Linux ビルドは一部で **macOS より先行** しています。Core は共通で
UI だけが Linux 専用のため、macOS の画面はこれからです:

- **ボックス（PC）.** タマゴを買うとポケモンは破棄されず、成長を保ったままボックスへ移ります。
  いつでも連れ出せますし、望めば自分の手でにがすこともできます。
- **日誌.** 孵化・進化・名づけ・ボックスの出入りを出来事として記録し、文章は読むときに作ります —
  言語を変えれば過去まで書き直されます。
- **実績.** カウンターではなくその記録への問い合わせなので、**さかのぼって** 反映されます。
- **トレーナーカード.** 自分で決めた名前と、共有用 PNG への **画像として保存**。
- **ニックネーム**、そしてクリックで **なでる**。
- **留守のあいだに** — 前回開いてから何があったかの要約。
- **キーボード操作** — `Ctrl+1`〜`Ctrl+5` でタブ切り替え、`Esc` で閉じる。

**動かないもの:**

2つは Wayland が禁じているため恒久的です — フローティングペットのドラッグ・位置の記憶と、
ウィンドウをトレイアイコンに吸着させたりフォーカス喪失で閉じたりすること。クライアントは自分の
サーフェス位置を知ることも決めることもできません。ペットの配置は KWin のウィンドウルールで行います。

残りは [ROADMAP.md](ROADMAP.md) で管理しています。
