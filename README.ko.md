...
**되는 것:** 애니메이션 companion 과 오늘 사용량이 붙은 트레이 아이콘, 홈/상점/가방/컬렉션/트레이너
창, 진화 라인, 리셋 카운트다운과 번레이트 예측이 붙은 공식 5시간·주간 한도 게이지, 페이지·필터·종별
상세가 있는 종 도감, 포획 로그, 호버 정보·우클릭 메뉴·말풍선 알림이 붙은 플로팅 펫, 데스크톱 알림,
세이브 내보내기·가져오기, 인앱 업데이트 확인, 설정. 사용량 파싱·companion 진행·세이브는 macOS 와
**같은 코드**입니다.

Linux 빌드는 최근 작업이 여기서 이뤄져 **일부 기능이 macOS 보다 앞서 있습니다.** Core 는 공유하고
UI 만 Linux 전용이라, macOS 화면은 아직 없습니다:

- **박스(PC).** 알을 사면 포켓몬이 폐기되지 않고 성장을 유지한 채 박스로 갑니다. 다시 꺼내거나,
  원하면 직접 놓아줄 수 있습니다.
- **일지.** 부화·진화·이름 짓기·박스 출입을 사건 그대로 기록해 문장은 읽을 때 만듭니다 — 언어를
  바꾸면 과거까지 다시 쓰입니다.
- **업적.** 카운터가 아니라 그 기록 위의 질의라 **소급 적용**됩니다.
- **트레이너 카드.** 직접 정한 이름과, 공유용 PNG **이미지로 저장**.
- **애칭**, 그리고 눌러서 **쓰다듬기**.
- **그동안 있었던 일** — 마지막으로 연 뒤 무슨 일이 있었는지 요약.
- **키보드 이동** — `Ctrl+1`~`Ctrl+5` 로 탭 전환, `Esc` 로 닫기.

**안 되는 것:**

둘은 Wayland 가 금지해서 영구적입니다 — 플로팅 펫의 드래그·위치 기억, 그리고 창을 트레이 아이콘에
붙이거나 포커스 상실 시 닫는 것. 클라이언트가 자기 표면 위치를 알거나 정할 수 없기 때문입니다.
펫 배치는 KWin 창 규칙으로 하세요.

나머지는 [ROADMAP.md](ROADMAP.md) 에서 추적합니다.
