...
read_when:
  ...
  - macOS 뷰를 다른 프론트엔드(GTK)로 옮기거나 그 포팅을 리뷰할 때 — 특히 되돌릴 수 없는 동작
---
...
## 플랫폼 포팅 (macOS 뷰 → GTK 재구현)

- **뷰 안에만 있는 규칙은 포팅에서 조용히 사라진다 — 특히 "한 번 더 묻기".** Linux 팝오버의 상점 알
  버튼이 첫 클릭에 곧바로 `buyEgg` 를 불러, 10시간 키운 동행이 확인 한 번 없이 사라졌다(2026-08-19,
  사용자 리포트). macOS 는 같은 동작을 `EggCard` 의 `@State private var stage` 2단계(이로치면 3단계)로
  막고 있었지만, 그 사다리는 **SwiftUI 뷰 안에만** 있었다. GTK 재구현은 "행을 그리고 버튼을 연결한다"는
  *레이아웃* 만 옮겼고 사다리는 옮기지 않았다 — 레이아웃은 눈으로 비교하면 차이가 보이는데,
  없는 확인 단계는 스크린샷에 **안 나타난다**(있어야 할 것의 부재는 비교 대상이 없다).
  같은 빌드에서 상점 아이템 구매(`buyConfirm`)와 가방 사용(`useOnCurrent`)도 같은 이유로 즉시 실행이었다
  — 하나가 빠지면 대개 부류 전체가 빠져 있다.
  - **규칙은 Core 의 순수 타입에 두고 두 프론트엔드가 읽는다**(`ActionConfirmPolicy`,
    `ImportConfirmPolicy` 와 같은 형태). GTK 위젯 트리는 XCTest 로 못 만들어 뷰 자체는 회귀를 못 잡는다 —
    **테스트 가능한 형태로 규칙을 끄집어내는 것이 회귀 가드를 만드는 유일한 방법**이다.
  - 회귀 가드(`ActionConfirmPolicyTests`)는 "단계가 몇 개인가"가 아니라 **"첫 클릭에 커밋되는가"** 를
    전수(모든 알 티어 × 이로치 여부 × 모든 아이템)로 본다. 결함을 주입해(`steps` → `[]`) 실패하는 것까지
    확인했다.
  - 남은 한계: macOS 뷰는 아직 자기 사다리를 손으로 들고 있다(맥에서 빌드·검증 불가). 정책 타입으로
    옮기기 전까지 두 구현이 갈라질 수 있다.
- **"값이 없으면 섹션을 숨긴다"는 포팅하면 안 되는 규칙이다 — 사라진 UI 와 고장난 UI 는 화면에서 같다.**
  Linux 팝오버가 `if let limits = store.limits` 로 한도 카드를 통째로 감춰, 429 백오프 중에 앱이
  재시작되면(한도는 메모리 캐시라 재시작으로 사라진다) 사용자에겐 기능이 없어진 것으로 보였다
  (2026-08-19 리포트: "예전엔 한도가 보였는데 지금은 안 보인다"). macOS 는 같은 상황에서 헤더와
  안내 행을 남긴다 — 즉 이것도 포팅에서 *설명 상태만* 빠진 경우다(위 항목과 같은 부류).
  - 실패 사유를 로그에만 쓰면 **화면에서 복구 불가능한 정보가 된다.** 자동 폴 경로도 사용자 문구를
    상태로 남긴다(`UsageStore.limitsErrorText`) — 수동 경로만 남기던 것이 공백이었다.
  - 회귀 가드는 **양쪽 브랜치**를 밟는다: 429 에서 사유가 남는지, 그리고 성공 후 사유가 지워지는지.
    실패만 보면 사유를 영영 안 지우는 구현(복구 뒤에도 429 안내가 붙어 있는)도 통과한다.
    복구 브랜치를 밟으려면 백오프를 건너뛸 수 있어야 해서 `resetLimitsBackoff()` 를 internal 로 열었다.
- **손수 쓴 `init(from:)` 이 있는 타입에 필드를 더하면, 인코딩은 되고 디코딩만 조용히 빠진다.**
  `CompanionState` 는 관대 디코더를 손으로 들고 있고 `CodingKeys` 는 합성이다 → 새 필드는 **저장은
  되는데 다음 기동에서 사라진다.** 저장도 로드도 "성공"하므로 `.corrupt` 복구도 안 걸린다. 박스(PC)를
  넣을 때 실제로 이 상태로 한 번 커밋 직전까지 갔다 — 개체를 **잃지 않으려고** 만든 기능이 개체를
  잃는 가장 그럴듯한 경로였다(Codex 리뷰가 잡았다).
  - 규칙: 손수 쓴 디코더가 있는 타입에 필드를 더하면 **같은 변경에서 디코드 줄을 넣고**, 회귀 가드는
    메모리 상태가 아니라 **파일을 다시 읽어** 확인한다(`BoxTests.testBoxSurvivesAReloadFromDisk`).
  - 컬렉션 필드는 `Lossy<T>` 항목별 격리를 쓴다(도감과 동일) — 통째 디코드면 개체 하나의 손상이
    컬렉션 전체를 날린다.
  - **세이브 스키마도 같이 올린다.** 안 올리면 구버전 앱이 schema 1 을 받아들이고 모르는 키를 무시한 뒤
    다음 저장에서 통째로 날린다(에러 없이). "못 읽는다"가 "읽고 지웠다"보다 낫다.
- **정규화 가드를 새 기능 때문에 "좁히기" 전에, 그 가드가 지키던 값이 어디로 갔는지 먼저 본다.**
  박스에서 개체를 꺼내면 품고 있던 알이 갈 곳을 잃는다(알은 `active == nil` 일 때만 인큐베이션된다).
  `sanitized` 의 "활성이 있으면 `eggTier` 는 유출" 가드를 heldEgg 예외로 좁히려 했지만, 정답은
  **가드를 그대로 두고 정당한 값을 명시적인 자리(`heldEgg`)로 옮기는 것**이었다. 좁히면 손편집·구버전
  조합의 stale 한 최상위 보증이 살아남아 졸업 후의 **무료** 알이 그걸 물려받는다.
  - 짝으로 온 결함: 그렇게 옆에 둔 알은 **졸업 경로**에서 되살려야 한다. `graduate()` 가 active 만
    비우면 `active == nil` + `heldEgg` 상태로 저장되고, 다음 기동의 sanitized 가 그걸 손상으로 보고
    지운다 — 산 알이 졸업 축하와 함께 증발한다. 새 상태를 넣을 땐 **그 상태를 벗어나는 모든 경로**를
    센다(꺼내기·되돌리기만 보고 졸업을 놓쳤다).
- **동작의 의미가 바뀌면 그 동작을 설명하는 문구와 확인 단계도 같이 바꾼다.** 알 구매가 폐기에서
  보관으로 바뀌자 "정말 놓아줄까요?"(이로치 경고)와 destructive 스타일이 **거짓**이 됐다. 없는 위험을
  경고하면 진짜 위험한 곳에서 그 경고의 무게가 사라진다 — 확인 단계는 공짜가 아니다.
- **포팅 완료 판정에 "파괴적 동작 목록"을 넣어라.** 새 프론트엔드를 얹을 때 화면 단위로 훑으면
  *보이는 것*(라벨·간격·아이콘)만 대조하게 된다. 되돌릴 수 없는 동작(개체 폐기·소모품 사용·세이브
  덮어쓰기)을 먼저 열거하고 각각이 **원본과 같은 횟수만큼 묻는지** 확인한다. 참고로 세이브 불러오기
  (`SettingsWindow` 의 `confirm`)는 이 방식으로 포팅돼 확인 단계가 살아남았다 — 빠진 건 팝오버뿐이었다.

## 파생 상태를 "기록 위의 질의"로 만들 때

업적·"그동안 있었던 일"처럼 **별도 카운터를 두지 않고 기록(일지·도감) 위의 질의로 만드는** 설계는
두 진실이 어긋나는 문제를 없애 준다. 대신 다음 두 가지가 새로 생긴다 — 둘 다 실제로 밟았다.

- **기록이 잘리면 판정이 뒤집힌다.** 일지는 200개 상한이라 오래 쓰면 오래된 사건이 밀려나고,
  근거가 그 사건뿐인 업적(밤의 사람·이름을 지어…)이 **조용히 다시 잠겼다**. 달성 취소는 사실의
  변화가 아니라 기록의 유실이다 → 한 번 달성한 것은 별도 집합(`earnedAchievements`)에 못박고
  판정의 첫 줄에서 그 집합을 먼저 본다. 같은 이유로 "여정 일수"도 잘린 일지에 기대면 줄어든다.
- **"기록이 비었다"로 첫 실행을 판단하지 마라.** 소급 달성분을 조용히 심으려고 "기록이 비었으면
  시드"로 판단했더니, **갓 시작한 세이브도 기록이 비어 있어** 신규 사용자의 첫 업적이 통째로
  삼켜졌다. "이 기능을 겪었나"는 별도 플래그(`achievementsSeeded`, `candyFeatureSeeded` 와 같은 부류)로
  들고, 데이터의 공백으로 추론하지 않는다.

## 상태 필드 추가 (세이브 스키마)

- **필드를 더할 때마다 `SaveEnvelope.schemaVersion` 을 올린다.** 박스 때 2로 올린 뒤 다섯 개
  (애칭·일지·트레이너 이름·업적·마지막 열람)를 그대로 얹었다. 그 사이 버전의 앱은 이 세이브를
  받아들이고 모르는 키를 무시한 뒤 다음 저장에서 통째로 날린다 — 버전을 두는 목적이 정확히 그
  손실을 막는 것인데, 안 올리면 아무 일도 안 하는 숫자가 된다.
  - 가드는 값만 비교하면 안 된다. 실제로 일어난 일은 "스키마는 그대로인데 필드만 늘어남"이므로,
    회귀 테스트가 **필드 수와 스키마 값을 함께** 고정한다(`testSaveSchemaIsBumpedWheneverStateGainsFields`).
- **새 필드는 `sanitized` 에도 등장해야 한다.** 인벤토리 개수는 클램프에서 빠져 있어 `Int.max` 가
  들어오면 사탕 지급의 `+=` 가 트랩나고, 그 변경은 저장되지 않으므로 **재기동해도 같은 파일로 또
  죽는다**. 외부 입력 규칙(§판정·데이터)이 수치 전부에 적용된다는 뜻이다.
- **콜백은 전이가 끝난 뒤에 부른다.** 졸업 알림 훅을 전이 도중에 불러서, 받는 쪽이 아직 남아 있는
  옛 활성 개체와 아직 안 세팅된 졸업 상태를 봤다. 상태를 바꾸는 함수에서 바깥으로 알리는 지점은
  **마지막 줄**이다.

## 캐시된 값과 엣지 트리거

- **과거 값을 화면에 되살리는 것과 판정에 먹이는 것은 다른 일이다.** 마지막 성공 한도를 디스크에서
  되살려 카드에 보여 주는 기능(G5)이, 같은 `limits` 필드를 통해 **사탕 지급·한도 알림**에도 흘러
  들어갔다. 지급은 "새로 100% 를 넘어선 순간"에만 발화하고 100% 미만을 보면 재무장하는 엣지
  트리거라, 캐시된 낮은 값이 이미 지급한 창을 재무장하면 다음 실제 조회에서 **두 번째 지급**이 나가고,
  캐시된 높은 값은 기동하자마자 알림을 쏜다(Codex 리뷰가 잡았다).
  - 규칙: 되살린 값에는 **출처 플래그**를 달고(`limitsAreRestored`), 엣지 판정은 실제 조회를 기다린다.
    "값이 있다"와 "판정해도 된다"를 같은 조건으로 쓰지 않는다.
  - 판정 규칙은 인스턴스 상태에서 떼어내 순수 함수로 만든다 — 안 그러면 테스트가 못 잡는다.
- **문서에 "표시 전용"이라고 써 두는 것은 강제가 아니다.** 최초 구현의 주석은 정확히 "표시 전용"이라고
  주장했지만 코드는 그렇지 않았다. 주장과 코드가 갈리면 주석이 리뷰어를 오히려 안심시킨다.

## 게임 보정과 실사용 수치

- **표시 단위가 무엇인지 정하고, 보정된 값을 그 단위인 척 보여 주지 마라.** 성격이 성장에 ±10% 를
  주자(G3) `usedAtStage` 는 보정된 XP 가 됐는데, "다음 단계까지 남은 양"은 그 차이를 그대로 토큰인
  것처럼 보여 줬다 — 느린 성격은 "100M 남음"이라 적히고 실제로는 111M 을 써야 한다. 화면이 약속하는
  단위(사용자가 실제로 쓰는 토큰)로 되돌려서 보여 준다.
- **값이 고정된 아이템은 보정에서 빼라.** 이상한 사탕(+100M)이 성격 배율을 타면서 실제로는 90M/110M 이
  적립됐고, 화면 피드백은 계속 "+100M" 이었다. 산 물건의 가치가 대상에 따라 달라지면 소모품이 아니라
  조견표가 된다.
- **기존 테스트가 균형 수치에 정확히 붙어 있으면 보정 도입에서 깨진다.** `graduationTotal` 을 **정확히**
  넣어 졸업시키던 정렬 테스트가 느린 성격에서 실패했다. 그 테스트가 검증하려던 건 순서였으므로 넉넉히
  넘기도록 고쳤다 — 균형 수치를 바꿀 때 어떤 테스트가 진짜로 균형을 검증하는지 구분하는 신호이기도 하다.

## 인덱스는 신원이 아니다

- **확인 창이나 재빌드를 건너 인덱스를 들고 있으면, 그 사이 목록이 바뀌었을 때 엉뚱한 대상에 적용된다.**
  박스에서 놓아주기가 그랬다: 확인 창이 떠 있는 동안 세이브 불러오기가 박스를 통째로 바꾸면, 확인은
  그 자리에 밀려 들어온 **다른 개체**를 되돌릴 수 없게 놓아준다. "인덱스가 범위 안"은 신원 검사가 아니다.
  - 규칙: 인덱스를 넘기는 API 는 **대상 값도 함께** 받아 확인한다(`release(at:expecting:)`).
    되돌릴 수 있는 동작(`withdraw`)에도 같은 가드를 둔다 — 비슷한 두 API 중 어느 쪽이 안전한지
    읽는 사람이 추론해야 하는 상황을 만들지 않는다.
  - 값(종 id·`ShopEntry`·`ItemKind`)을 캡처하는 콜백은 이 문제가 없다. 위험한 것은 **위치**를 캡처할 때다.
- **페이지 인덱스는 양쪽 끝을 다 자른다.** 탭을 떠나면 0으로 되돌리는데 옛 페이지 위젯은 그대로 남아
  있어서, 그 "이전" 버튼이 0에서 한 번 더 빼 음수 슬라이스로 트랩났다. 상한만 자르고 하한을 잊는 것이
  전형적인 형태다.

## 스크롤 영역 안의 입력 위젯 (GTK)

- **스크롤을 "값 변경"으로 받는 위젯은 스크롤 페이지 안에서 조용히 설정을 바꾼다.** 설정 창을 마우스
  휠로 내리다 슬라이더 위를 지나가면 그 설정이 바뀐다 — 실제로 플로팅 펫 크기가 96 → 48 로 바뀌었고
  화면에는 아무 표시도 없었다. `GtkScale` 과 `GtkComboBox` 가 모두 해당하고, 언어 콤보 위를 지나가면
  앱 언어가 바뀐다.
  - **한 곳만 고치면 안 된다 — 부류다.** 스크롤 영역 안의 모든 스크롤 감응 위젯에 같은 처리를 한다.
  - 해결은 이벤트를 **삼키는 것이 아니라 넘기는 것**이다(`gtkConnectScrollPassthrough`): 위젯에서
    가장 가까운 `GtkScrolledWindow` 를 찾아 그 vadjustment 를 직접 움직이고 TRUE 를 돌려준다.
    그냥 막으면 페이지가 안 굴러가는 죽은 구역이 생겨 놀라움을 다른 놀라움으로 바꿀 뿐이다.
  - 검증은 "스크롤한 뒤 **저장된 값이 그대로인지**" 로 한다. 화면만 보면 값이 바뀐 걸 못 알아챈다.

## GTK CSS (Linux 스타일시트)

- **같은 특정도면 시트에서 나중에 선언된 규칙이 이긴다 — 그래서 "덧칠용" 클래스는 대상 클래스보다
  뒤에 있거나 더 구체적이어야 한다.** 희귀도 배지(`.ptb-rarity-*`)를 `.ptb-badge` 보다 **앞에** 선언한
  탓에, 배경색은 배지의 회색에 지고 글자색만 살아남아 **흰 글자 + 회색 배경**이 됐다. 한 규칙의 두
  속성 중 하나만 적용된 상태라 "클래스가 안 붙었나?"로 오해하기 쉽다.
  - 해결: 복합 선택자(`.ptb-badge.ptb-rarity-legendary`)로 특정도를 올린다 — 시트 순서가 바뀌어도
    안전하다. 순서에 기대는 수정은 다음 편집에서 조용히 되돌아간다.
- **`alpha()` 로 칠한 배경은 테마를 탄다.** 뒤에 깔린 색과 섞이므로, 다크에서 고른 반투명 색이 라이트
  에서는 물빠진 색이 된다. 브랜드 색처럼 **양쪽 테마에서 같아야 하는 칠**은 불투명색 + 명시적 글자색으로
  둔다(글자색을 테마에 맡기면 밝은 칠 위에 밝은 글자가 나온다).
- **다크 테마만 보고 색을 고르지 마라.** 이 저장소의 CSS 는 전부 다크에서 눈으로 맞춘 것이라,
  라이트 테마에서 `#d08770` 같은 경고색이 대비 미달이었다. 검증 방법은 `.claude/NOTES.md` 의
  격리 프로필(`HOME` 분리) — 그쪽이 GTK 기본 테마(라이트)로 뜬다.
